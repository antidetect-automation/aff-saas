...
async function sendWelcome(env, chatId, source, from) {
  ...
  await tg(env, "sendMessage", {
    chat_id: chatId,
    text: "Quick bar ready — SAAS50 · MIN50 · Deal · Pricing · Playwright · API · Errors · Ads · Cloud Phone · Help · Cloud Phone · Help",
...
async function askAI(env, chatId, question) {
  ...
  const prompt = [
    ...
    "If asked about ads: point to Facebook/Google/affiliate use-cases and isolation hygiene — never ban guarantees.",
...
async function ensureBotMeta(env) {
  ...
  const commands = [
    ...
    { command: "cloudphone", description: "Cloud Phone use-case" },
